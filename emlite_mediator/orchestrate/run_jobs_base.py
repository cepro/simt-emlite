...
class RunJobForAllMeters():
    ...
    def run(self):
        ...
        if (len(registry_result.data) == 0):
            self.logger.error("no meters record found")
            sys.exit(11)
        ...
        for chunk_idx in list(range(0, num_chunks)):
            ...
            for meter in chunk_meters:
                ...
                if (mediator_container == None):
                    self.logger.error(
                        "NO CONTAINER EXISTS FOR meter [%s]", meter['id'])
                    continue
                ...
                self.logger.info('run for meter=%s mediator port=%s',
    ...
    def _check_environment(self):
        if not mediator_image:
            self.logger.error("MEDIATOR_IMAGE not set to a docker image.")
            sys.exit(1)

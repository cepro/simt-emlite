...
from simt_emlite.jobs.util import handle_mediator_unknown_failure, handle_meter_unhealthy_status, handle_supabase_faliure, update_meter_shadows_when_healthy
from simt_emlite.util.supabase import supa_client, Client as SupabaseClient
...
class SyncerBase(ABC):
    ...
    def __init__(
        ...
        self.log = logger.bind(
            meter_id=meter_id, syncer=self.__class__.__name__)
    ...
    def _update_shadow(self, update_props: Dict[str, any]):
        self.log.info("update shadow props", meter_id=self.meter_id,
    ...
    def _update_registry(self, update_props: Dict[str, any]):
        self.log.info("update registry props",

...
public class EmlitePacket implements Serializable, Encodable {
    ...
    private EmlitePacket(int dst, int src, byte control, byte format, ObisId id, Integer profileReadTime, byte read,
            byte[] value) {
        this.dst = dst;
        this.src = src;
    ...
    public static int checkPacket(ByteBuffer buf) {
        ...
        if (!EMLITE_CRC.validate(bytes)) {
            ...
            log.error("CRC-16-CCITT does not match; received {}, calculated {}", Format.toHex(expectedCRC),
                    Format.toHex(calcCRC));
            return BAD_CRC;
        }
        return 0;
    }
